...
Change IP Address:
Although you can change the IP Address in the YahooNewsApp, its easier to hardcode the default to your own IP address. To do this, change the "server_ip" variable in the ReaderMainActivity.java file if you are using the YahooNewsApp. If you choose to run the test app (AndroidImageTest), you will need to change the "mIP" variable in both the AudioClient.java and SocketClient.java files. 
...
HOW TO RUN:
To run this program, first run the MainProgram in OpenCVTest (not a good name for now). Then run the android app (either the test app AndroidImageTest or the YahooNewsApp). 